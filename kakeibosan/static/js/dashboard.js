dashboard = {
  initDashboardPageCharts: function(total_per_months, costs_per_month, utility_costs) {
    ...
    var myChart = new Chart(ctx, {
      type: 'line',
      data: {
        labels: total_per_months['months'],
        datasets: [{
          label: "出費合計",
    ...
    // パーセンテージ大きい順にソート
    costs_per_month.sort(function(a, b){
    ...
    for(let i in costs_per_month){
        categories.push(costs_per_month[i]['category']);
        percentage = Math.round((costs_per_month[i]['amount'] / total_per_months['total'].slice(-1)[0]) * 100);
